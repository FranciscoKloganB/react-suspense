...
In this lesson we learned how to use `React.useTransition` hook to temporarely
control the loading state, before letting `React.Suspense` take over with the
fall back component. This is important to improve the user experience. Suppose,
the request that fetches the pokemon does not take that long... Then perhaps,
our loading indicator can be a simple fade out, instead of immediatly showing
the "Loading pokemon..." message. This is cool and easy to use. Unfortunatly,
the configuration that we could give to useTransition, no longer receives
a `SUSPENSE_CONFIG`, so we must deal with transition timeouts in `userland`. :)
