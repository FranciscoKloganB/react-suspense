...
In this exercise we refactored the Pokemon application to use the
`React.Suspense` API and benefitted from that, since managing errors, fallbacks
and the actual pokemon is simpler when using this approach _vs_ the one we had
when using the regular `React.useEffect` + `Fetch On Render` approach. However,
we still are not `Rendering as We Fetch`.

Finally, we learned, that `React.Suspense`, just like `ErrorBoundary` replaces
the entire subtree with the `fallback` prop componet until the data is loaded!
