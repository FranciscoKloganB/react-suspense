...
On this exercise we learned how to Suspend and preload images. This is important
because some images may be heavy, which slows down the user experience. However,
when we started Suspending the image, we ran into problems where the pokemon
data was being displayed before the pokemon image. So, noticing that we actually
had a way of getting the pokemon image at the same time as the data, we split
the single-render-as-you-fetch into two different flows. One to fetch the data
and another to fetch the image. This sped up the experience while keeping the
benefit of not displaying the pokemon data before the image.

Finally, since, we are suspending our component, and fetching the required
resources in parallel, we can actually Lazy load the component that uses that
data. In this case, the difference is not very noticeable, but if the lazy
loaded component was heavy in logic, then it would be! The reason why we want
to lazy load when we `render-as-you-fetch`, is due to the fact that this
technique is managed on some parent component, which means we lose the benefits
of state collocation.
